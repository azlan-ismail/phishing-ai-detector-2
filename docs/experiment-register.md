...
## Manuscript reframing and evidence-linked tables

The [replacement draft](manuscript-revision.md) uses the latest matched-budget results as its principal comparison and keeps earlier diagnostics budget-specific. Tables were generated from per-seed metrics, with 360 metric-summary pairs checked against the verified summary; evidence is in research/audits/manuscript-table-verification.json. The [reviewer coverage map](manuscript-revision-review.md) records unresolved requests. No new training runs or result rows were added, and the experiment artifact inventory is unchanged.
